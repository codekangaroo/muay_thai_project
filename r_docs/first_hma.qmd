...
Andy Field's tutorials for r (very handy but not mandatory). First install:
...
```{r}
hma_tib <- here::here("data/hma_data_long_v.csv") |> readr::read_csv()
```
